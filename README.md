...
### plugin-free Vite config
- `vite.config.js` intentionally avoids `@vitejs/plugin-react` import to prevent CI branch drift from breaking builds.
- This removes the exact `ERR_MODULE_NOT_FOUND` failure mode shown in build logs.


### setup-node npm cache behavior
- This workflow intentionally does **not** set `cache: npm` in `actions/setup-node`.
- Reason: GitHub Actions fails early if npm cache is enabled but no `package-lock.json` exists.
- After lockfile is committed, enabling npm cache is safe and recommended.
