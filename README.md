...
### CI dependency safeguard
- CI explicitly runs an install guard for `@vitejs/plugin-react` before `npm run build`.
- This prevents intermittent `ERR_MODULE_NOT_FOUND` failures when merged branch states have dependency drift.
