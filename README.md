...
- CI includes an explicit guard step to ensure `@vitejs/plugin-react` exists before `vite build` runs.
- This prevents build failures from stale/partial lockfile states after branch merges.
